...
pub fn validate_state_transition(from: TransactionState, to: TransactionState) -> Result<(), String> {
    match (from, to) {
        ...
        _ => {
            Err(format!("Invalid state transition from {:?} to {:?}", from, to))
        }
    }
}
...
pub fn validate_transaction_state_consistency(transaction: &Transaction) -> Result<(), String> {
    match transaction.state() {
        ...
        TransactionState::PartiallySigned => {
            ...
            if transaction.signatures.is_empty() {
                return Err("PARTIALLY_SIGNED transaction must have at least one signature".to_string());
            }
            if transaction.recent_blockhash.is_empty() {
...
pub fn validate_operation_allowed_for_state(state: TransactionState, operation: &str) -> Result<(), String> {
    match (state, operation) {
        ...
        _ => {
            Err(format!("Operation '{}' not allowed for transaction state {:?}", operation, state))
        }
    }
}
...
mod tests {
    ...
    fn test_valid_state_transitions() {
        ...
        assert!(validate_state_transition(TransactionState::Draft, TransactionState::Compiled).is_ok());
        assert!(validate_state_transition(TransactionState::Compiled, TransactionState::PartiallySigned).is_ok());
        assert!(validate_state_transition(TransactionState::Compiled, TransactionState::FullySigned).is_ok()); // Direct
        assert!(validate_state_transition(TransactionState::PartiallySigned, TransactionState::FullySigned).is_ok());
        
        // Valid self-transitions (for adding more signatures)
        assert!(validate_state_transition(TransactionState::PartiallySigned, TransactionState::PartiallySigned).is_ok());
    }

    #[test]
    fn test_invalid_state_transitions() {
        // Cannot go backward to DRAFT
        assert!(validate_state_transition(TransactionState::Compiled, TransactionState::Draft).is_err());
        assert!(validate_state_transition(TransactionState::PartiallySigned, TransactionState::Draft).is_err());
        assert!(validate_state_transition(TransactionState::FullySigned, TransactionState::Draft).is_err());
        
        // Cannot go backward from PARTIALLY_SIGNED to COMPILED
        assert!(validate_state_transition(TransactionState::PartiallySigned, TransactionState::Compiled).is_err());
        
        // Cannot transition from FULLY_SIGNED (terminal state)
        assert!(validate_state_transition(TransactionState::FullySigned, TransactionState::Compiled).is_err());
        assert!(validate_state_transition(TransactionState::FullySigned, TransactionState::PartiallySigned).is_err());
        
        // Cannot transition from/to UNSPECIFIED
        assert!(validate_state_transition(TransactionState::Unspecified, TransactionState::Draft).is_err());
        assert!(validate_state_transition(TransactionState::Draft, TransactionState::Unspecified).is_err());
    }
    ...
    fn test_operation_permissions() {
        ...
        assert!(validate_operation_allowed_for_state(TransactionState::Compiled, "estimate").is_ok());
        assert!(validate_operation_allowed_for_state(TransactionState::Compiled, "simulate").is_ok());
        assert!(validate_operation_allowed_for_state(TransactionState::Compiled, "compile").is_err());
        assert!(validate_operation_allowed_for_state(TransactionState::Compiled, "submit").is_err());

        // PARTIALLY_SIGNED operations
        assert!(validate_operation_allowed_for_state(TransactionState::PartiallySigned, "sign").is_ok()); // Add more sigs
        assert!(validate_operation_allowed_for_state(TransactionState::PartiallySigned, "estimate").is_ok());
        assert!(validate_operation_allowed_for_state(TransactionState::PartiallySigned, "compile").is_err());
        assert!(validate_operation_allowed_for_state(TransactionState::PartiallySigned, "submit").is_err());

        // FULLY_SIGNED operations
        assert!(validate_operation_allowed_for_state(TransactionState::FullySigned, "submit").is_ok());
        assert!(validate_operation_allowed_for_state(TransactionState::FullySigned, "estimate").is_ok());
        assert!(validate_operation_allowed_for_state(TransactionState::FullySigned, "simulate").is_ok());
        assert!(validate_operation_allowed_for_state(TransactionState::FullySigned, "compile").is_err());
        assert!(validate_operation_allowed_for_state(TransactionState::FullySigned, "sign").is_err());

        // UNSPECIFIED - no operations allowed
        assert!(validate_operation_allowed_for_state(TransactionState::Unspecified, "compile").is_err());
        assert!(validate_operation_allowed_for_state(TransactionState::Unspecified, "sign").is_err());
        assert!(validate_operation_allowed_for_state(TransactionState::Unspecified, "submit").is_err());
    }

    #[test]
    fn test_edge_cases() {
        // Invalid operation names
        assert!(validate_operation_allowed_for_state(TransactionState::Draft, "invalid_operation").is_err());
        assert!(validate_operation_allowed_for_state(TransactionState::Compiled, "").is_err());
        
        // Test error messages are descriptive
        let result = validate_state_transition(TransactionState::FullySigned, TransactionState::Draft);

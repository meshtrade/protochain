...
impl SystemProgramService for SystemProgramServiceImpl {
    ...
    async fn initialize_nonce_account(
        ...
        let instruction = instructions.into_iter().nth(1)
    ...
    async fn authorize_nonce_account(
        ...
        let current_authority = Pubkey::from_str(&req.current_authority)
            .map_err(|e| Status::invalid_argument(format!("Invalid current authority address: {}", e)))?;

//! Generic conversion utilities for Solana SDK types and protobuf types
//!
//! This module provides common conversions between Solana SDK types (Instruction, AccountMeta)
...
///
/// This function transforms an instruction from the protobuf representation
/// used in gRPC APIs to the native Solana SDK format used for transactions.
///
/// # Arguments
/// * `instruction` - The protobuf instruction to convert
///
/// # Returns
/// * `Ok(Instruction)` - Successfully converted instruction
/// * `Err(String)` - Error message if conversion fails (e.g., invalid `program_id` or pubkeys)

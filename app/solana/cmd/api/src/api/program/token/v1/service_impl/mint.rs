//! Handler implementation for `Mint` (`MintToChecked`).

use std::str::FromStr;

use tonic::{Request, Response, Status};

use protochain_api::protochain::solana::program::token::v1::{MintRequest, MintResponse};
...
use spl_token_2022::{
    extension::StateWithExtensions, instruction::mint_to_checked, state::Mint,
...
impl TokenProgramServiceImpl {
    ...
    ///
    /// Reads the mint account on-chain to resolve the token program, mint
    /// authority, and decimal precision.  The destination ATA is derived from
    ...
    pub(crate) async fn handle_mint(
        ...
        // Unpack the mint to read authority and decimals.
        ...
        let mint_authority_pubkey = mint.mint_authority.ok_or_else(|| {
            Status::failed_precondition("Mint has no mint authority (minting is disabled)")
        })?;

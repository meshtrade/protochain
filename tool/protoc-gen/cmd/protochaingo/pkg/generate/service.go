...
func Service(p *protogen.Plugin, f *protogen.File, svc *protogen.Service) error {
	...
	g.P("//")
	g.P("// 1. PROTOCHAIN_API_CREDENTIALS environment variable")
	g.P("//")
	g.P("// 2. Default credential file location:")
	g.P("//")
	g.P("//   - Linux:   $XDG_CONFIG_HOME/protochain/credentials.json or fallback to $HOME/.config/protochain/credentials.json")
	g.P("//   - macOS:   $HOME/Library/Application Support/protochain/credentials.json")
	g.P("//   - Windows: C:\\\\Users\\\\<user>\\\\AppData\\\\Roaming\\\\protochain\\\\credentials.json")
	g.P("//")

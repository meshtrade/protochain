...
echo "🏃 Generating TypeScript (ts-web) Bindings"
echo "============================================"
echo ""
...
# Check if the buf.gen.ts-web.yaml exists
if [ ! -f "${PROJECT_ROOT}/lib/_code_gen/buf.gen.ts-web.yaml" ]; then
    echo "❌ Error: buf.gen.ts-web.yaml not found at ${PROJECT_ROOT}/lib/_code_gen/buf.gen.ts-web.yaml"
    exit 1
fi
...
cd "${PROJECT_ROOT}"
if ! buf generate "lib/proto" --template "lib/_code_gen/buf.gen.ts-web.yaml"; then
...
echo ""
echo "✅ TypeScript (ts-web) code generation complete!"
echo ""
